...
def gpt_socket(personality):
    ...
    try:
        ...
        if lock.locked():
            logEvent(username=determineUser(request), ip = determineIP(request), subject="/gpt-socket/"+personality, eventtype="contention")
        with lock:
...
def toil(personality):
    ...
    if lock.locked():
        logEvent(username=determineUser(request), ip = determineIP(request), subject="/toil/"+personality, eventtype="contention")
